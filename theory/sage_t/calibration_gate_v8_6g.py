"""SAGE.T8.6g minimum-KL family-mass projection gate."""
...
from . import calibration_gate_v8_6f as v86f
...
FORMAT_VERSION = "sage-t8.6g-minimum-kl-family-v1"
DEFAULT_MANIFEST_PATH = Path(__file__).with_name(
    "sage_t8_6g_selection_manifest.json"
)
DEFAULT_OUTPUT_DIR = Path("training") / "sage_t" / "calibration_v8_6g"
DEFAULT_PARENT_MANIFEST = v86f.DEFAULT_MANIFEST_PATH
DEFAULT_PARENT_REPORT = v86f.DEFAULT_OUTPUT_DIR / "selection_report.json"
DEFAULT_ACTION_SCHEDULES = v86f.DEFAULT_ACTION_SCHEDULES
CHALLENGERS = (
...
def _code_hashes() -> dict[str, str]:
    ...
    return {
        ...
        for name in ("posterior_v8.py", "calibration_gate_v8_6g.py")
    }
...
def _load_parent_report(path: str | Path) -> dict[str, Any]:
    ...
    if checksum != v86c._checksum(unsigned):
        raise ValueError("parent T8.6f report checksum mismatch")
    if payload.get("status") != "T8_6F_SELECTION_FAILED_CLOSED":
        raise ValueError("T8.6g requires the failed T8.6f selection")
    if payload.get("selected_challenger") is not None:
        raise ValueError("T8.6f already selected a challenger")
    if payload.get("source_validation_authorized") is not False:
        raise ValueError("T8.6f source-validation firewall is open")
    return payload
...
def freeze_manifest(
    *,
    ...
    parent_manifest = v86f.load_manifest(parent_manifest_path)
    ...
    if len(schedules) != int(parent_manifest["corpus"]["roots"]):
        raise ValueError("T8.6g schedules do not cover every root")
    payload: dict[str, Any] = {
        "format_version": FORMAT_VERSION,
        "status": "FROZEN_BEFORE_T8_6G_SOURCE_TRAIN_SELECTION",
        "frozen_at": "2026-08-06",
        "parent_t8_6f_manifest_checksum": parent_manifest[
            "manifest_checksum"
        ],
        "parent_t8_6f_scientific_checksum": parent_report[
            "scientific_checksum"
        ],
        ...
        "policies": {
            ...
            for name, policy in T8_6G_POLICIES.items()
        },
        ...
        "frozen_invariants": {
            ...
            "objective": "minimize_KL_q_parallel_p",
            "constraint": "normalized_particle_entropy>=0.0501",
            "optimization_domain": "semantic_family_masses",
            ...
            "only_inference_change": "minimum_kl_family_mass_projection",
        },
...
def load_manifest(
    ...
    *,
    ...
    if checksum != v86c._checksum(unsigned):
        raise ValueError("SAGE.T8.6g manifest checksum mismatch")
    if payload.get("format_version") != FORMAT_VERSION:
        raise ValueError("unsupported SAGE.T8.6g manifest")
    if payload.get("status") != "FROZEN_BEFORE_T8_6G_SOURCE_TRAIN_SELECTION":
        raise ValueError("SAGE.T8.6g manifest is not frozen")
    parent_manifest = v86f.load_manifest(parent_manifest_path)
    parent_report = _load_parent_report(parent_report_path)
    if payload.get("parent_t8_6f_manifest_checksum") != parent_manifest.get(
        "manifest_checksum"
    ):
        raise ValueError("T8.6g parent manifest drifted")
    if payload.get("parent_t8_6f_scientific_checksum") != parent_report.get(
        "scientific_checksum"
    ):
        raise ValueError("T8.6g parent scientific result drifted")
    if payload.get("corpus") != v86.EXPECTED_CORPUS:
        raise ValueError("T8.6g corpus contract drifted")
    ...
    if firewall.get("authority") != "shadow" or any(
        ...
        raise ValueError("T8.6g firewall is open")
    if verify_hashes:
        if payload.get("code_sha256") != _code_hashes():
            raise ValueError("T8.6g code drifted")
        if payload.get("action_schedules_sha256") != v86c._file_sha256(
            Path(schedules_path)
        ):
            raise ValueError("T8.6g action schedules drifted")
    return payload
...
@contextmanager
def _minimum_kl_runner() -> Iterable[None]:
...
def _projection_summary(
    ...
    for condition in T8_6G_POLICIES:
        ...
        alphas = [
            float(row.get("projection_alpha", 1.0)) for row in projected
        ]
        output[condition] = {
            ...
            "mean_projection_alpha": mean(alphas) if alphas else 1.0,
...
def run_selection(
    *,
    ...
    if inventory != v86.EXPECTED_CORPUS:
        raise ValueError(f"T8.6g corpus integrity failure: {inventory}")
    ...
    with _minimum_kl_runner():
        for episode in episodes:
            ...
            for policy in T8_6G_POLICIES.values():
                ...
                if observed_keys != keys:
                    raise RuntimeError("T8.6g action sequence drifted")
    ...
    if winner is not None:
        repair_policy = T8_6G_POLICIES[winner].with_repair_v2()
        with _minimum_kl_runner():
            for episode in episodes:
                keys = [str(value) for value in schedules[episode.episode_id]]
                for dynamics_only in (False, True):
                    ...
                    if observed_keys != keys:
                        raise RuntimeError("T8.6g Repair V2 schedule drifted")
    ...
    report: dict[str, Any] = {
        "format_version": FORMAT_VERSION,
        "status": (
            "T8_6G_SELECTION_FAILED_CLOSED"
            if winner is None
            else (
                "READY_FOR_T8_6G_LIVE_CONFIRMATION"
                if bool((offline_confirmation or {}).get("passed"))
                else "T8_6G_REPAIR_V2_FAILED_CLOSED"
            )
        ),
        "manifest_checksum": manifest["manifest_checksum"],
        "parent_t8_6f_scientific_checksum": manifest[
            "parent_t8_6f_scientific_checksum"
        ],
        "inventory": inventory,
        "roots": len(episodes),
        ...
        "conditions": list(T8_6G_POLICIES),
