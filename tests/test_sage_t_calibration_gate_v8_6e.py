...
from theory.sage_t.calibration_gate_v8_6e import (
    CHALLENGERS,
    DEFAULT_ACTION_SCHEDULES,
    _floor_runner,
    _new_floor_posterior,
    freeze_manifest,
    load_manifest,
    select_challenger,
)
from theory.sage_t.posterior_v6 import T8_6E_POLICIES


def test_manifest_binds_t8_6d_and_only_adds_minimal_entropy_floor(
    tmp_path: Path,
) -> None:
    ...
    assert invariants["entropy_floors"] == [0.0501, 0.0525, 0.0550]
    assert invariants["maximum_family_mixture"] == 0.02
    assert invariants["projection_solver"] == "minimum_mixture_binary_search"
    assert loaded["firewall"]["source_validation_opened"] is False
    assert loaded["firewall"]["authority"] == "shadow"


def test_t8_6e_reuses_the_exact_parent_schedule() -> None:
...
def test_floor_runner_restores_frozen_factory() -> None:
    original = v86._new_posterior

    with _floor_runner():
        assert v86._new_posterior is _new_floor_posterior
...
def test_selection_preserves_every_t8_6_gate(tmp_path: Path) -> None:
    ...
    winner_name = "terminal_tempered_20_family_floor_0501"
    rows = []
