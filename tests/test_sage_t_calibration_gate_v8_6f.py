...
from theory.sage_t.calibration_gate_v8_6f import (
    CHALLENGERS,
    DEFAULT_ACTION_SCHEDULES,
    _conditional_runner,
    _new_conditional_posterior,
    freeze_manifest,
    load_manifest,
    select_challenger,
)
from theory.sage_t.posterior_v7 import T8_6F_POLICIES


def test_manifest_binds_t8_6e_and_freezes_only_reference_change(
    tmp_path: Path,
) -> None:
    ...
    assert (
        invariants["within_family_reference"]
...
def test_t8_6f_reuses_exact_parent_schedule() -> None:
...
def test_conditional_runner_restores_frozen_factory() -> None:
    original = v86._new_posterior

    with _conditional_runner():
        assert v86._new_posterior is _new_conditional_posterior
