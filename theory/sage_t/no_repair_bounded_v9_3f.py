"""SAGE.T9.3f bounded confirmation with live repair disabled."""
...
from . import efficient_bounded_v9_3e as efficient
from . import live_shadow_pilot as live_base
from . import live_shadow_pilot_v7 as live_i
from . import winning_prefix_gate_common as prefix_gate
...
FORMAT_VERSION = "sage-t9.3f-no-repair-bounded-v1"
DEFAULT_MANIFEST_PATH = Path(__file__).with_name(
    "sage_t9_3f_no_repair_bounded_manifest.json"
)
DEFAULT_PARENT_REPORT = efficient.DEFAULT_OUTPUT_DIR / "report.json"
DEFAULT_OUTPUT_DIR = Path("training") / "sage_t" / "bounded_v9_3f"
NO_REPAIR_CAPS: Mapping[str, int] = {
    **dict(efficient.EFFICIENT_CAPS),
    "maximum_repair_contexts": 0,
}
...
def _code_hashes() -> dict[str, str]:
    ...
    return {
        ...
        "no_repair_bounded_v9_3f.py": v86c._file_sha256(
            directory / "no_repair_bounded_v9_3f.py"
        ),
        "winning_prefix_gate_common.py": v86c._file_sha256(
            directory / "winning_prefix_gate_common.py"
        ),
    }
...
def _load_parent_report(
    ...
    if checksum != v86c._checksum(unsigned):
        raise ValueError("T9.3e report checksum mismatch")
    if report.get("status") != "T9_3E_FAILED_CLOSED":
        raise ValueError("T9.3f requires the failed-closed T9.3e gate")
    checks = dict(report.get("checks", {}))
    if {name for name, passed in checks.items() if not passed} != {
        "observation_p95"
    }:
        raise ValueError("T9.3e must have failed only observation latency")
    ...
    if (
        ...
        raise ValueError("T9.3e lacks complete safe-progress evidence")
    if report.get("t9_4_authorized") is not False:
        raise ValueError("T9.3e did not remain fail closed")
    return report


class NoRepairBoundedController(efficient.EfficientBoundedController):
    """Efficient controller whose repair budget is explicitly zero."""


def build_controller(manifest: Mapping[str, Any]) -> NoRepairBoundedController:
    ...
    return NoRepairBoundedController(
        executor=executor,
        posterior=posterior,
...
def freeze_manifest(
    *, output_path: str | Path = DEFAULT_MANIFEST_PATH
    ...
    parent = efficient.load_manifest()
    parent_report = _load_parent_report()
    audit = prefix_gate.audit_winning_prefixes(
        NO_REPAIR_CAPS,
    ...
    if not audit["passed"]:
        raise ValueError("no-repair controller failed the winning-prefix audit")
    payload: dict[str, Any] = {
        "format_version": FORMAT_VERSION,
        "status": "FROZEN_BEFORE_T9_3F_NO_REPAIR_BOUNDED_SOURCE_TRAIN",
        "frozen_at": "2026-08-06",
        "parent_t9_3e_manifest_checksum": parent["manifest_checksum"],
        "parent_t9_3e_report_checksum": parent_report["report_checksum"],
        "code_sha256": _code_hashes(),
        "controller_caps": dict(NO_REPAIR_CAPS),
        "winning_prefix_audit": audit,
        ...
        "registered_changes": [
            "maximum live repair contexts 16 to 0",
            "deterministic generation and exact posterior updates retained",
            "repair implementation retained behind a rollback flag",
        ],
        "gate": dict(parent["gate"]),
...
def load_manifest(path: str | Path = DEFAULT_MANIFEST_PATH) -> dict[str, Any]:
    ...
    if checksum != v86c._checksum(unsigned):
        raise ValueError("T9.3f manifest checksum mismatch")
    if payload.get("format_version") != FORMAT_VERSION:
        raise ValueError("unsupported T9.3f manifest")
    if payload.get("status") != "FROZEN_BEFORE_T9_3F_NO_REPAIR_BOUNDED_SOURCE_TRAIN":
        raise ValueError("T9.3f manifest is not frozen")
    if payload.get("code_sha256") != _code_hashes():
        raise ValueError("T9.3f code drifted")
    if payload.get("controller_caps") != dict(NO_REPAIR_CAPS):
        raise ValueError("T9.3f caps drifted")
    if not bool(payload.get("winning_prefix_audit", {}).get("passed")):
        raise ValueError("T9.3f winning-prefix audit did not pass")
    if payload.get("parent_t9_3e_manifest_checksum") != efficient.load_manifest().get(
        "manifest_checksum"
    ):
        raise ValueError("T9.3e manifest drifted")
    if payload.get("parent_t9_3e_report_checksum") != _load_parent_report().get(
        "report_checksum"
    ):
        raise ValueError("T9.3e report drifted")
    ...
    if any(
        ...
        raise ValueError("T9.3f firewall is open")
    return payload
...
def run_pilot(
    *,
    ...
    return gate_common.run_bounded_gate(
        ...
        controller_builder=build_controller,
        format_version=FORMAT_VERSION,
        passed_status="T9_3F_PASSED",
        failed_status="T9_3F_FAILED_CLOSED",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    return 0 if args.freeze or result.get("status") == "T9_3F_PASSED" else 2
