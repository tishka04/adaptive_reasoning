"""CLI for preregistered SAGE.T12.4a.4d target-local search."""
...
from .target_regrounding_protocol import freeze_target_regrounding

DEFAULT_ROOT = Path(__file__).resolve().parents[3]
DEFAULT_OUTPUT = (
    DEFAULT_ROOT / "training" / "sage_t" / "target_regrounding_t12_4a_4d_bp35"
)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    freeze = subparsers.add_parser(
        "freeze",
        help="seal the guarded posterior, exact anchors, shield and paired design",
    )
    freeze.add_argument("--parent-manifest", required=True)
    freeze.add_argument("--parent-receipt", required=True)
    freeze.add_argument("--witness-registry", required=True)
    freeze.add_argument("--shield-manifest", required=True)
    freeze.add_argument("--shield-receipt", required=True)
    ...
    run = subparsers.add_parser(
        "run",
        help="run paired generic and contract-re-grounded search from level 1",
    )
    ...
    status = subparsers.add_parser("status", help="verify T12.4a.4d artifacts")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        if args.phase == "freeze":
            ...
            result = target_regrounding_status(
                manifest_path=args.manifest,
        ...
        result = {
            "format_version": "sage-t12.4a.4d-target-regrounding-cli-error-v1",
            "phase": args.phase,
