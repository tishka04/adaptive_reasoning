"""CLI for preregistered SAGE.T12.5b observed-effect shadow ranking."""
...
from .progress_shadow_experiment import progress_shadow_status, run_progress_shadow
...
def build_parser() -> argparse.ArgumentParser:
    ...
    freeze = subparsers.add_parser(
        "freeze", help="seal the passed T12.5 posterior and paired replay design"
    )
    ...
    run = subparsers.add_parser(
        "run", help="collect fixed exact-prefix branches and score them in shadow"
    )
    run.add_argument("--manifest", default=str(DEFAULT_OUTPUT / "manifest.json"))
    run.add_argument("--output-dir", default=str(DEFAULT_OUTPUT / "shadow"))
    run.add_argument("--environments-dir", default=str(DEFAULT_ROOT / "environment_files"))

    status = subparsers.add_parser("status", help="verify T12.5b artifacts and gate")
