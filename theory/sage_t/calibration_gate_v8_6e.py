"""SAGE.T8.6e minimal collapse-triggered family-entropy-floor gate."""
...
from . import calibration_gate_v8_6d as v86d
...
FORMAT_VERSION = "sage-t8.6e-adaptive-family-floor-v1"
DEFAULT_MANIFEST_PATH = Path(__file__).with_name(
    "sage_t8_6e_selection_manifest.json"
)
DEFAULT_OUTPUT_DIR = Path("training") / "sage_t" / "calibration_v8_6e"
DEFAULT_PARENT_MANIFEST = v86d.DEFAULT_MANIFEST_PATH
DEFAULT_PARENT_REPORT = v86d.DEFAULT_OUTPUT_DIR / "selection_report.json"
DEFAULT_ACTION_SCHEDULES = v86d.DEFAULT_ACTION_SCHEDULES
CHALLENGERS = (
...
def _code_hashes() -> dict[str, str]:
    ...
    return {
        ...
        for name in ("posterior_v6.py", "calibration_gate_v8_6e.py")
    }
...
def _load_parent_report(path: str | Path) -> dict[str, Any]:
    ...
    if checksum != v86c._checksum(unsigned):
        raise ValueError("parent T8.6d report checksum mismatch")
    if payload.get("status") != "T8_6D_SELECTION_FAILED_CLOSED":
        raise ValueError("T8.6e requires the failed T8.6d selection")
    if payload.get("selected_challenger") is not None:
        raise ValueError("T8.6d already selected a challenger")
    if payload.get("source_validation_authorized") is not False:
        raise ValueError("T8.6d source-validation firewall is open")
    return payload
...
def freeze_manifest(
    *,
    ...
    parent_manifest = v86d.load_manifest(parent_manifest_path)
    ...
    if len(schedules) != int(parent_manifest["corpus"]["roots"]):
        raise ValueError("T8.6e schedules do not cover every root")
    payload: dict[str, Any] = {
        "format_version": FORMAT_VERSION,
        "status": "FROZEN_BEFORE_T8_6E_SOURCE_TRAIN_SELECTION",
        "frozen_at": "2026-08-06",
        "parent_t8_6d_manifest_checksum": parent_manifest[
            "manifest_checksum"
        ],
        "parent_t8_6d_scientific_checksum": parent_report[
            "scientific_checksum"
        ],
        ...
        "frozen_invariants": {
            ...
            "entropy_floors": [0.0501, 0.0525, 0.0550],
            ...
            "only_inference_change": (
                "minimal_collapse_triggered_family_entropy_floor"
            ),
        },
...
def load_manifest(
    ...
    *,
    ...
    if checksum != v86c._checksum(unsigned):
        raise ValueError("SAGE.T8.6e manifest checksum mismatch")
    if payload.get("format_version") != FORMAT_VERSION:
        raise ValueError("unsupported SAGE.T8.6e manifest")
    if payload.get("status") != "FROZEN_BEFORE_T8_6E_SOURCE_TRAIN_SELECTION":
        raise ValueError("SAGE.T8.6e manifest is not frozen")
    parent_manifest = v86d.load_manifest(parent_manifest_path)
    parent_report = _load_parent_report(parent_report_path)
    if payload.get("parent_t8_6d_manifest_checksum") != parent_manifest.get(
        "manifest_checksum"
    ):
        raise ValueError("T8.6e parent manifest drifted")
    if payload.get("parent_t8_6d_scientific_checksum") != parent_report.get(
        "scientific_checksum"
    ):
        raise ValueError("T8.6e parent scientific result drifted")
    if payload.get("corpus") != v86.EXPECTED_CORPUS:
        raise ValueError("T8.6e corpus contract drifted")
    ...
    if firewall.get("authority") != "shadow" or any(
        ...
        raise ValueError("T8.6e firewall is open")
    if verify_hashes:
        if payload.get("code_sha256") != _code_hashes():
            raise ValueError("T8.6e code drifted")
        if payload.get("action_schedules_sha256") != v86c._file_sha256(
            Path(schedules_path)
        ):
            raise ValueError("T8.6e action schedules drifted")
    return payload


def _new_floor_posterior(
    ...
    *,
    ...
    return AdaptiveFamilyFloorProgramPosterior(
        executor=executor,
...
@contextmanager
def _floor_runner() -> Iterable[None]:
    original = v86._new_posterior
    v86._new_posterior = _new_floor_posterior
    try:
        yield
    finally:
...
def _diagnostic_conclusion(
    ...
    selected = [
        ...
        == (winner or "terminal_tempered_20_family_floor_0501")
    ]
...
def _projection_summary(
    ...
    for condition in T8_6E_POLICIES:
...
def run_selection(
    *,
    ...
    if inventory != v86.EXPECTED_CORPUS:
        raise ValueError(f"T8.6e corpus integrity failure: {inventory}")
    ...
    with _floor_runner():
        for episode in episodes:
            ...
            for policy in T8_6E_POLICIES.values():
                ...
                if observed_keys != keys:
                    raise RuntimeError("T8.6e action sequence drifted")
    ...
    if winner is not None:
        repair_policy = T8_6E_POLICIES[winner].with_repair_v2()
        with _floor_runner():
            for episode in episodes:
                keys = [str(value) for value in schedules[episode.episode_id]]
                for dynamics_only in (False, True):
                    ...
                    if observed_keys != keys:
                        raise RuntimeError("T8.6e Repair V2 schedule drifted")
    ...
    report: dict[str, Any] = {
        "format_version": FORMAT_VERSION,
        "status": (
            "T8_6E_SELECTION_FAILED_CLOSED"
            if winner is None
            else (
                "READY_FOR_T8_6E_LIVE_CONFIRMATION"
                if bool((offline_confirmation or {}).get("passed"))
                else "T8_6E_REPAIR_V2_FAILED_CLOSED"
            )
        ),
        "manifest_checksum": manifest["manifest_checksum"],
        "parent_t8_6d_scientific_checksum": manifest[
            "parent_t8_6d_scientific_checksum"
        ],
        "inventory": inventory,
        "roots": len(episodes),
        ...
        "conditions": list(T8_6E_POLICIES),
        ...
        "adaptive_projection": projection_summary,
