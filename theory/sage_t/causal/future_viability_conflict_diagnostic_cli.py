"""CLI for the offline SAGE.T12.6.1a transition-conflict diagnostic."""
...
from .future_viability_conflict_diagnostic_experiment import (
...
def build_parser() -> argparse.ArgumentParser:
    ...
    freeze = subparsers.add_parser(
        "freeze", help="freeze post-hoc conflict policies and boundaries"
    )
    freeze.add_argument("--parent-manifest", required=True)
    freeze.add_argument("--parent-evaluation-receipt", required=True)
    freeze.add_argument("--manifest", default=str(DEFAULT_OUTPUT / "manifest.json"))

    diagnose = subparsers.add_parser(
        "diagnose", help="audit conflicts under fixed consolidation policies"
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.phase == "freeze":
        manifest = freeze_future_viability_conflict_diagnostic(
            ...
            parent_evaluation_receipt_path=args.parent_evaluation_receipt,
        )
        print(json.dumps(manifest, indent=2, sort_keys=True))
        return 0 if manifest["firewall"]["diagnostic_authorized"] else 3
    if args.phase == "diagnose":
        receipt = run_future_viability_conflict_diagnostic(
            manifest_path=args.manifest,
            output_dir=args.output_dir,
        )
        print(json.dumps(receipt, indent=2, sort_keys=True))
        return 0 if receipt["passed"] else 3
    status = future_viability_conflict_diagnostic_status(
        manifest_path=args.manifest,
