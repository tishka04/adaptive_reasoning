...
from theory.p3.risk_targeted_contextual_post_stop_policy_validation import (
    ...
    classify_risk_targeted_safe_stops,
    run_risk_targeted_contextual_post_stop_policy_validation,
    validate_p3g3_source,
)
...
def _m3g4_source() -> dict:
    return {
        ...
        "per_safe_stop_validation_records": [
            {
                "safe_stop_id": "seen_m3g4",
                "safe_stop_state_hash": "state::m3g4",
                "captured_prefix_hash": "prefix::m3g4",
                "candidate_records": [],
            }
        ],
        "support": 0,
...
def _p3g3_source(*, support: int = 0) -> dict:
    return {
        "summary": {
            "policy_utility_status": "OUT_OF_SAMPLE_POLICY_UTILITY_CANDIDATE_ONLY",
            "accepted_out_of_sample_safe_stops": 1,
            "support": support,
        },
        "accepted_out_of_sample_safe_stops": [
            {
                "safe_stop_id": "seen_p3g3",
                "safe_stop_state_hash": "state::p3g3",
                "captured_prefix_hash": "prefix::p3g3",
            }
        ],
        "policy_utility_status": "OUT_OF_SAMPLE_POLICY_UTILITY_CANDIDATE_ONLY",
        "support": support,
...
def _plan(plan_id: str, action: str) -> SafeStopCandidatePlan:
    extra = {
        "seen_p3g3": [],
        "non_target": ["ACTION3"],
        "risk_blocked": ["ACTION4"],
        "risk_allowed": ["ACTION6"],
    }.get(plan_id, [])
    return SafeStopCandidatePlan(
        plan_id=plan_id,
        planned_prefix=tuple(
            {"action": value, "action_args": {}} for value in [action, *extra]
        ),
...
def _cell_result(cell, captured: CapturedSafeStop) -> dict:
    ...
    if cell.condition_kind == "hold":
        ...
        taps = hold + 5.0
        terminal = False
    elif safe_stop_id == "p3_g4::risk_oos_safe_stop::001":
        taps = 0.0
        terminal = True
    else:
        taps = hold + 15.0
...
def test_risk_targeted_validation_finds_risk_and_selector_avoids_it(
    tmp_path: Path,
) -> None:
    ...
    p3g3_path = _write_json(tmp_path / "p3g3.json", _p3g3_source())
    plans = [
        _plan("seen_p3g3", "ACTION3"),
        _plan("non_target", "ACTION4"),
        _plan("risk_blocked", "ACTION6"),
        _plan("risk_allowed", "ACTION3"),
    ]

    def fake_candidate_executor(plan: SafeStopCandidatePlan) -> dict:
        if plan.plan_id == "seen_p3g3":
            return _candidate_result(
                plan,
                family="seen",
                horizon=50,
                hold=140.0,
                state_hash="state::p3g3",
                prefix_hash="prefix::p3g3",
            )
        if plan.plan_id == "non_target":
            return _candidate_result(
                plan,
                family="new_low",
                horizon=60,
                hold=40.0,
                state_hash="state::low",
                prefix_hash="prefix::low",
            )
        if plan.plan_id == "risk_blocked":
            return _candidate_result(
                plan,
                family="blocked_family",
                horizon=50,
                hold=140.0,
                state_hash="state::risk_blocked",
                prefix_hash="prefix::risk_blocked",
            )
        return _candidate_result(
            plan,
            family="new_family",
            horizon=53,
            hold=100.0,
            state_hash="state::risk_allowed",
            prefix_hash="prefix::risk_allowed",
        )
    ...
    payload = run_risk_targeted_contextual_post_stop_policy_validation(
        ...
        source_p3g3_path=p3g3_path,
        min_risk_targeted_safe_stops=2,
    ...
    assert payload["summary"]["accepted_risk_targeted_safe_stops"] == 2
    assert payload["summary"]["in_sample_or_previous_oos_safe_stops_rejected"] == 1
    assert payload["summary"]["non_target_risk_contexts_rejected"] == 1
    assert decisions["p3_g4::risk_oos_safe_stop::001"] == ACTION6_ONLY
    assert decisions["p3_g4::risk_oos_safe_stop::002"] == ACTION6_ACTION3
    assert payload["summary"]["static_extension_terminal_options"] == 2
...
def test_risk_targeted_classifier_rejects_non_target_context() -> None:
    record = {
        "execution_performed": True,
        "replay_exact": True,
        "non_terminal": True,
        "terminal_safe": True,
        "hold_baseline_measurable": True,
        "hold_baseline_terminal_adjusted_progress": 40.0,
        "terminal_horizon_estimate": {"estimated_moves_remaining": 60},
        "safe_stop_state_hash": "state::fresh",
        "captured_prefix_hash": "prefix::fresh",
        "relation_state_signature": "relation::fresh",
    }

    classified, accepted = classify_risk_targeted_safe_stops(
        [record],
        excluded_identity={"state_hashes": set(), "prefix_hashes": set()},
        min_risk_targeted_safe_stops=1,
    )

    assert accepted == []
    assert classified[0]["risk_targeted_acceptance_status"] == NON_TARGET_RISK_CONTEXT_REJECTED
    assert classified[0]["support"] == 0


def test_p3g3_source_rejects_support() -> None:
    with pytest.raises(ValueError):
        validate_p3g3_source(_p3g3_source(support=1))
