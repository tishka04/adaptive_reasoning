"""Prospective chronological collector for SAGE12 V4.1."""
...
COLLECTION_FORMAT_VERSION = "sage12-mechanic-collection-v4.1"
...
def _collect_game(
    *,
    ...
    reset_offset = max((row.reset_index for row in records), default=-1) + 1
    raw_steps = resets_used = chronological_repeats = 0
    ...
    if len(records) != target_rows:
        raise RuntimeError(
            f"SAGE12 V4.1 collection incomplete for {game}: "
...
def _stratum(trace: ActionTargetTrace) -> str:
    return (
        f"{trace.selected_action_name}|{trace.anchor.action_family}|"
        f"{trace.anchor.kind}|{trace.anchor.path_status}"
    )
