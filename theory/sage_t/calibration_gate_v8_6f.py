"""SAGE.T8.6f posterior-conditional family projection gate."""
...
from . import calibration_gate_v8_6e as v86e
from .executor import ProgramExecutor
from .posterior_v6 import AdaptiveFamilyFloorPolicy
from .posterior_v7 import (
    T8_6F_POLICIES,
    PosteriorConditionalFamilyFloorProgramPosterior,
)

FORMAT_VERSION = "sage-t8.6f-posterior-conditional-family-v1"
DEFAULT_MANIFEST_PATH = Path(__file__).with_name(
    "sage_t8_6f_selection_manifest.json"
)
DEFAULT_OUTPUT_DIR = Path("training") / "sage_t" / "calibration_v8_6f"
DEFAULT_PARENT_MANIFEST = v86e.DEFAULT_MANIFEST_PATH
DEFAULT_PARENT_REPORT = v86e.DEFAULT_OUTPUT_DIR / "selection_report.json"
DEFAULT_ACTION_SCHEDULES = v86e.DEFAULT_ACTION_SCHEDULES
CHALLENGERS = (
...
def _code_hashes() -> dict[str, str]:
    ...
    return {
        ...
        for name in ("posterior_v7.py", "calibration_gate_v8_6f.py")
    }
...
def _load_parent_report(path: str | Path) -> dict[str, Any]:
    ...
    if checksum != v86c._checksum(unsigned):
        raise ValueError("parent T8.6e report checksum mismatch")
    if payload.get("status") != "T8_6E_SELECTION_FAILED_CLOSED":
        raise ValueError("T8.6f requires the failed T8.6e selection")
    if payload.get("selected_challenger") is not None:
        raise ValueError("T8.6e already selected a challenger")
    if payload.get("source_validation_authorized") is not False:
        raise ValueError("T8.6e source-validation firewall is open")
    return payload
...
def freeze_manifest(
    *,
    ...
    parent_manifest = v86e.load_manifest(parent_manifest_path)
    ...
    if len(schedules) != int(parent_manifest["corpus"]["roots"]):
        raise ValueError("T8.6f schedules do not cover every root")
    payload: dict[str, Any] = {
        "format_version": FORMAT_VERSION,
        "status": "FROZEN_BEFORE_T8_6F_SOURCE_TRAIN_SELECTION",
        "frozen_at": "2026-08-06",
        "parent_t8_6e_manifest_checksum": parent_manifest[
            "manifest_checksum"
        ],
        "parent_t8_6e_scientific_checksum": parent_report[
            "scientific_checksum"
        ],
        ...
        "policies": {
            ...
            for name, policy in T8_6F_POLICIES.items()
        },
        ...
        "frozen_invariants": {
            ...
            "projection_solver": "minimum_mixture_binary_search",
            "family_target": "uniform_semantic_family_mass",
            "within_family_reference": (
                "current_posterior_conditional_distribution"
            ),
            "within_family_ratio_invariant": True,
            ...
            "only_inference_change": (
                "preserve_posterior_ratios_inside_semantic_families"
            ),
        },
...
def load_manifest(
    ...
    *,
    ...
    if checksum != v86c._checksum(unsigned):
        raise ValueError("SAGE.T8.6f manifest checksum mismatch")
    if payload.get("format_version") != FORMAT_VERSION:
        raise ValueError("unsupported SAGE.T8.6f manifest")
    if payload.get("status") != "FROZEN_BEFORE_T8_6F_SOURCE_TRAIN_SELECTION":
        raise ValueError("SAGE.T8.6f manifest is not frozen")
    parent_manifest = v86e.load_manifest(parent_manifest_path)
    parent_report = _load_parent_report(parent_report_path)
    if payload.get("parent_t8_6e_manifest_checksum") != parent_manifest.get(
        "manifest_checksum"
    ):
        raise ValueError("T8.6f parent manifest drifted")
    if payload.get("parent_t8_6e_scientific_checksum") != parent_report.get(
        "scientific_checksum"
    ):
        raise ValueError("T8.6f parent scientific result drifted")
    if payload.get("corpus") != v86.EXPECTED_CORPUS:
        raise ValueError("T8.6f corpus contract drifted")
    ...
    if firewall.get("authority") != "shadow" or any(
        ...
        raise ValueError("T8.6f firewall is open")
    if verify_hashes:
        if payload.get("code_sha256") != _code_hashes():
            raise ValueError("T8.6f code drifted")
        if payload.get("action_schedules_sha256") != v86c._file_sha256(
            Path(schedules_path)
        ):
            raise ValueError("T8.6f action schedules drifted")
    return payload


def _new_conditional_posterior(
    policy: AdaptiveFamilyFloorPolicy,
    *,
    ...
    return PosteriorConditionalFamilyFloorProgramPosterior(
        executor=executor,
...
@contextmanager
def _conditional_runner() -> Iterable[None]:
    original = v86._new_posterior
    v86._new_posterior = _new_conditional_posterior
    try:
        yield
    finally:
...
def _projection_summary(
    ...
    for condition in T8_6F_POLICIES:
        selected = [row for row in updates if row["condition"] == condition]
        applied = [
            float(row.get("applied_family_mixture", 0.0))
            for row in selected
            if bool(row.get("regularization_applied"))
        ]
        output[condition] = {
            "projections": len(applied),
            "mean_applied_mixture": mean(applied) if applied else 0.0,
            "maximum_applied_mixture": max(applied, default=0.0),
            "floor_shortfalls": sum(
                bool(row.get("regularization_applied"))
                and not bool(row.get("floor_reached"))
                for row in selected
            ),
        }
    return output
...
def _diagnostic_conclusion(
    ...
    selected = [
        ...
        for row in rows
        if row.get("condition") == (winner or CHALLENGERS[0])
    ]
...
def run_selection(
    *,
    ...
    if inventory != v86.EXPECTED_CORPUS:
        raise ValueError(f"T8.6f corpus integrity failure: {inventory}")
        ...
        for episode in episodes:
            ...
            for policy in T8_6F_POLICIES.values():
                ...
                if observed_keys != keys:
                    raise RuntimeError("T8.6f action sequence drifted")
    ...
    if winner is not None:
        repair_policy = T8_6F_POLICIES[winner].with_repair_v2()
        with _conditional_runner():
            for episode in episodes:
                keys = [str(value) for value in schedules[episode.episode_id]]
                for dynamics_only in (False, True):
                    ...
                    if observed_keys != keys:
                        raise RuntimeError("T8.6f Repair V2 schedule drifted")
    ...
    report: dict[str, Any] = {
        "format_version": FORMAT_VERSION,
        "status": (
            "T8_6F_SELECTION_FAILED_CLOSED"
            if winner is None
            else (
                "READY_FOR_T8_6F_LIVE_CONFIRMATION"
                if bool((offline_confirmation or {}).get("passed"))
                else "T8_6F_REPAIR_V2_FAILED_CLOSED"
            )
        ),
        "manifest_checksum": manifest["manifest_checksum"],
        "parent_t8_6e_scientific_checksum": manifest[
            "parent_t8_6e_scientific_checksum"
        ],
        "inventory": inventory,
        "roots": len(episodes),
        ...
        "conditions": list(T8_6F_POLICIES),
        ...
        "posterior_conditional_projection": projection_summary,
    ...
    report["scientific_checksum"] = v86c._checksum(
        {
            ...
            for key in (
                ...
                "posterior_conditional_projection",
                "failure_taxonomy",
                "conclusion",
